...
uniform vec4 green;
void main(){
    out_color = green;//vec4(v_color, 1.0);
}
...
while not glfw.window_should_close(window):
    ...
    #上传uniform颜色 
    t  = glfw.get_time()
    greencolor = sin(t)/2.0+0.5
    redcolor = cos(t)/2.0 +0.5
    location = glGetUniformLocation(shade,"green")
    glUseProgram(shade)
    glUniform4f(location,redcolor,greencolor,0.0,1.0)
